...
class PDF extends FPDF
{
	...
	function GenerarTicketCierre(&$pdf, $data_)
	{ 
		// CABECERA
		$pdf->SetFont('Helvetica', '', 12);
		$pdf->Cell(60, 1, 'CEVICHERIA WILLY GOURMET', 0, 1, 'C');
		$pdf->SetFont('Helvetica', 'B', 11);
		$pdf->Cell(60, 6, 'Cierre del dia ' . $data_[0]['fecha'], 0, 1, 'C');
		$pdf->Image($pdf->pathFirma, 27, 17, 30, 0, 'PNG');
		...
		$date = new DateTime($data_[0]['fecha']);
		$timestamp = $date->getTimestamp();

		$total = intval($data_[0]['YAPE']) + intval($data_[0]['VISA']) + intval($data_[0]['EFECTIVO']) + intval($data_[0]['PLIN']);

		$dias = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado', 'domingo'];
		$numeroDia = getdate($timestamp);
		$nombreDia = $dias[$numeroDia['wday']];
		$pdf->SetFont('Helvetica', 'B', 11); 
		$pdf->Cell(60, 4, 'Fecha     :  ' . $data_[0]['fecha'], 0, 1, '');
		$pdf->Ln(2); 
		$pdf->Cell(60, 4, 'Dia          :  ' . utf8_decode($nombreDia), 0, 1, '');
		$pdf->Ln(2);  
		$pdf->Cell(60, 4, 'Yape       :  ' . $data_[0]['YAPE'], 0, 1, '');
		$pdf->Ln(2); 
		$pdf->Cell(60, 4, 'Plin         :  ' . $data_[0]['PLIN'], 0, 1, '');
		$pdf->Ln(2);  
		$pdf->Cell(60, 4, 'Efectivo  :  ' . $data_[0]['EFECTIVO'], 0, 1, '');
		$pdf->Ln(2); 
		$pdf->Cell(60, 4, 'Visa         :  ' . $data_[0]['VISA'], 0, 1, '');
		$pdf->Ln(2); 
 		$pdf->Cell(0, 3, '- - - - - - - - - - - - - - - - - - - - - - - - -', 0);
		$pdf->Ln(2); 
		$pdf->Ln(2); 
		...
		$pdf->Cell(60, 4, 'Total        :  ' . $total , 0, 1, '');
		$pdf->Ln(2); 
		$pdf->Ln(2); 

		$pdf->SetFont('Helvetica', 'B', 13); 

		$pdf->Cell(60, 4, 'Gastos        :  ', 0, 1, '');
	...
	function Footer()
	{
		...
		if ($this->function == 'GenerarTicketCliente') {
	...
	function GenerarTicketCocina(&$pdf, $data_)
	{
		...
		$pdf->Cell(-5, 0, 'Cant', 0, 1, 'C');
		$pdf->Cell(53, 0, 'Plato', 0, 0, 'C');
		$pdf->Cell(5, 0, 'S/', 0, 0, 'R');
		$pdf->Ln(0.5);
		...
		foreach ($data_ as $row) {
			if ($row['pedido_estado'] != 1) {
				...
				$pdf->Cell(13, 0, $row['cantidad'], 0, 1, 'C');
				$pdf->SetFont('Arial', 'I', 13);

				$pdf->Cell(53, 0, $row['acronimo'], 0, 0, 'L');
				$pdf->Cell(7, 0, $row['precioU'], 0, 0, 'L');
				$pdf->SetTextColor(0, 0, 0);
				$pdf->Cell(13, 0, ($row['lugarpedido'] == 1 ? 'Mesa' : 'Llevar'), 0, 0, 'R');
				$pdf->Ln();
...
class reporteDetalle
{
	...
	function imprimirticketCierre($data_)
	{
		$inicial = 140;  
		...
		$pdf->SetDisplayMode("fullpage");
		$pdf->AddPage();
		$pdf->GenerarTicketCierre($pdf, $data_);
		$pdf->Output(); 
		return $pdf;
	}
